...
class M_gedung extends CI_Model{
    ...
    {
        return $this->db->get_where($this->_table, ["kode_gedung" => $id])->row();
    }
    ...
    public function save($ven = null)
    {
        ...
        $kode = 'GD';
        $this->kode_gedung = $kode.floor(microtime(true)*321);
        $this->name = $post["name"];
        if ($ven != null) {
            $this->id_vendor = $ven;
        }else{
    ...
    public function update()
    {
        ...
        $this->detail = $post["detail"];
        $this->db->update($this->_table, $this, array('kode_gedung' => $post['id']));
    }

    public function delete($id)
    {
        $this->_deleteImage($id);
        return $this->db->delete($this->_table, array("kode_gedung" => $id));
    }
    ...
    private function _deleteImage($id){
        $gedung = $this->getById($id);
        if ($gedung->img != "default.jpg") {
            $filename = explode(".", $gedung->img)[0];
