...
class M_makeup extends CI_Model{
    ...
    {
        return $this->db->get_where($this->_table, ["kode_makeup" => $id])->row();
    }
    ...
    public function save()
    {
        ...
        $kode = 'MK';
        $this->kode_makeup = $kode.floor(microtime(true)*321);
        $this->name = $post["name"];
    ...
    public function update()
    {
        ...
        $this->detail = $post["detail"];
        $this->db->update($this->_table, $this, array('kode_makeup' => $post['id']));
    }

    public function delete($id)
    {
        $this->_deleteImage($id);
        return $this->db->delete($this->_table, array("kode_makeup" => $id));
    }
    ...
    private function _deleteImage($id){
        ...
        if ($makeup->img != "default.jpg") {
            $filename = explode(".", $makeup->img)[0];
